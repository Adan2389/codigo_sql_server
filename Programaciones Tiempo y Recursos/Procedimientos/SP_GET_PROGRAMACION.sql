...
ALTER PROCEDURE SELECT_PROGRAMACION (@ProgramacionID varchar(50))
...
BEGIN
	...
	DECLARE crsProgramacion CURSOR FOR 
		...
		FROM PLANNING_PROGRAMACION AS a WHERE ProgramacionID = @ProgramacionID
		ORDER BY Item ASC
	...
	BEGIN
		...
		BEGIN
			...
			begin
				if (@crs_TipoItem=1)
				begin
					if(@crs_FechaItem = @Temp_FechaItem )
					begin
						if (@crs_Turno = @Temp_Turno )	
						begin
							if (@crs_Descripcion = @Temp_Descripcion  )	
							begin
								if (@crs_TipoTiempo = @Temp_TipoTiempo)	
								begin
									Set @Guardar = 0
								end
								else
	...
	SELECT ROW_NUMBER() OVER (ORDER BY FechaItem ASC )AS Item, *, (SELECT DBO.TURNO_VIEW (Horario_TrabajoID, Turno ))as TurnoView
