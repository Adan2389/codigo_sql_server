...
--EXECUTE SELECT_LAMINADO '1__Prueba '

alter PROCEDURE SELECT_LAMINADO (@ProgramacionID varchar(50))
...
BEGIN
	...
	CREATE TABLE ##Temp_Programacion_Extrusion   (
		Item smallint,
	...
	INSERT INTO ##Temp_Programacion_Extrusion
		EXECUTE SELECT_PROGRAMACION @ProgramacionID

		--AJUSTO LA SALIDA DE LOS CAMPOS
		
		SELECT Item, FechaItem, Turno, [Horario Trabajo], Descripcion, DesProducto, Libras, Ancho, Largo, Espesor,
			   PesoMetro, (Libras/PesoMetro)as TotalMetros, ((LibrasXhora/PesoMetro)/60) as MetrosMetaMin, 
			   LibrasXhora, HorasTrabajo, TInicio, TFinal, IOL, TI
		FROM(
			SELECT  Item, FechaItem, Turno_View as Turno, Horario_TrabajoID as [Horario Trabajo],
					 ...
					 --Libras,
					 Ancho, Largo, Espesor, (SELECT USER_4 FROM VMFGPN.DBO.WORK_ORDER WHERE BASE_ID=Orden) as  PesoMetro,
					 MetrosMetaMin, LibrasXhora, HorasTrabajo,TInicio, TFinal, Item_Order_Line AS IOL, TipoItem as TI
			FROM ##Temp_Programacion_Extrusion as a
		)AS C1

	DROP TABLE  ##Temp_Programacion_Extrusion
